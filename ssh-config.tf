locals {
  template = {
    bastion  = var.create_bastion ? tomap({ ip = openstack_networking_floatingip_associate_v2.bastion[0].floating_ip, name = openstack_compute_instance_v2.bastion[0].name }) : ({ip="",name=""})
